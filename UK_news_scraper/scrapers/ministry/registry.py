...
from ...config import AGENCIES, DEFAULT_MAX_WORKERS, TOPIC_RULES
...
class FetchAllResult:
    ...
    def all_successful(self) -> bool:
        return all(status.success for status in self.statuses)
...
def fetch_all_with_status(since: datetime, max_workers: int = DEFAULT_MAX_WORKERS) -> FetchAllResult:
    ...
    failed_scrapers: list[tuple[AgencyFeedScraper, str]] = []

    with ThreadPoolExecutor(max_workers=workers) as executor:
        future_map = {}
        for scraper in scrapers:
            print(f"[info] 排入抓取 {scraper.agency.display_name}")
            future_map[executor.submit(scraper.fetch, since)] = scraper
        ...
        for future in as_completed(future_map):
            ...
            try:
                ...
                failed_scrapers.append((scraper, str(exc)))
                continue
    ...
    if failed_scrapers:
        ...
        with ThreadPoolExecutor(max_workers=retry_workers) as executor:
            future_map = {
                executor.submit(scraper.fetch, since): (scraper, first_error)
                for scraper, first_error in failed_scrapers
            }

            for future in as_completed(future_map):
                scraper, first_error = future_map[future]
                try:
                    items = future.result()
                except Exception as exc:
